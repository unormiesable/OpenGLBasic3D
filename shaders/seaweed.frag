...
float caustic_hash(vec2 p) {
    return fract(sin(dot(p, vec2(193.5, 271.9))) * 43758.5453);
}
...
float caustic_voronoi(vec2 uv) {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            ...
            vec2 r = cell + g;
            vec2 jitter = vec2(caustic_hash(r), caustic_hash(r + 37.73));
...
void main() {
    ...
    color += vec3(0.07, 0.12, 0.12) * projected_caustic(frag_pos, N);
